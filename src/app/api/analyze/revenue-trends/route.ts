...
import {
  AnalysisFramework,
  UnifiedAIAnalysisService,
} from '@/lib/shared/unified-ai-analysis.service';
...
const REVENUE_TRENDS_FRAMEWORK: AnalysisFramework = {
  name: 'Revenue Trends',
  description:
    'Analyze content for revenue opportunities, market trends, and growth potential using data-driven insights',
  elements: [
    'market_demand',
    'trending_keywords',
    'seasonal_patterns',
  ...
  try {
    ...
    if (result.success) {
      return NextResponse.json({
        ...
        ...result.data,
        message:
          'Revenue Trends analysis completed successfully using unified AI analysis service',
      });
    } else {
      return NextResponse.json(
        {
