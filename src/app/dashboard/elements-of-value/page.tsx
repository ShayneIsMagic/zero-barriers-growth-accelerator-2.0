...
export default function ElementsOfValuePage() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      try {
        const { data: b2cData } = await apiCall<ElementsAnalysis>(
        ...
        // B2C analysis may be unavailable; keep current behavior
      }

      // Fetch B2B Elements of Value analysis
      try {
        const { data: b2bData } = await apiCall<ElementsAnalysis>(
        ...
        // B2B analysis may be unavailable; keep current behavior
      }
    } catch (err) {
