...
export class ContentAnalyzer {
  async analyzeContent(
    ...
    const overallScore = Math.round(
      (goldenCircle.overallScore +
        elementsOfValue.overallScore +
        cliftonStrengths.overallScore) /
        3
    );
    ...
    const specificInsights = this.generatePageSpecificInsights(
      content,
      pageType
    );
  ...
  private analyzeGoldenCircle(content: string, pageType: string) {
    ...
    const overallScore = this.calculateGoldenCircleScore(
      whyText,
      howText,
      whatText,
      whoText
    ...
    return {
      why: {
        ...
        insights: [
          whyText.includes('not clearly defined')
            ? 'Purpose and mission need clarification'
            : 'Clear purpose statement identified',
        ],
      },
      how: {
        methodology: howText,
        framework: howText.includes('methodology')
        ...
        insights: [
          howText.includes('not clearly defined')
            ? 'Methodology needs better definition'
            : 'Clear approach identified',
        ],
      },
      what: {
        offerings: whatText.includes('not clearly defined') ? [] : [whatText],
        categories: whatText.includes('not clearly defined')
          ? []
          : ['Services'],
        score: Math.round(
          this.scoreElement(content, [
        ...
        insights: [
          whatText.includes('not clearly defined')
            ? 'Product/service descriptions need improvement'
            : 'Clear offerings identified',
        ],
      },
      who: {
        ...
        targetAudience: whoText.includes('not clearly defined')
        ...
        insights: [
          whoText.includes('not clearly defined')
            ? 'Client testimonials and target audience need better definition'
  ...
  private extractWhy(content: string, pageType: string): string {
    ...
    if (!why) {
      const companyDesc = content.match(
        /(?:apple|company|we|our).{0,100}(?:design|create|develop|build|innovate).{0,50}/gi
      );
  ...
  private extractHow(content: string, pageType: string): string {
    ...
    const methodologies = content.match(
      /(Attitude Cycle|IMPROV Sales Methodology|Purpose-Driven Exercise)[^.]*?\./gi
    );
    if (methodologies) {
  ...
  private extractWhat(content: string, pageType: string): string {
    ...
    const services = content.match(
      /(Human Transformation|Technology Enablement|Revenue Acceleration)[^.]*?\./gi
    );
    if (services) {
      what = services.join(' ');
    }
    ...
    const offerings = content.match(
      /(Custom software development|Salesforce implementation|Sales process optimization)[^.]*?\./gi
    );
    if (offerings) {
      what = offerings.join(' ');
    }
  ...
  private extractWho(content: string, pageType: string): string {
    ...
    const successMetrics = content.match(
      /(\d+% growth|\d+% ROI|\d+% increase)[^.]*?\./gi
    );
    ...
    const targetAudience = content.match(
      /(businesses|companies|organizations|customers|users).{0,50}/gi
    );
    ...
    return (
      cleanWho ||
      'Target audience and client testimonials not clearly defined in content'
    );
  }
  ...
  private analyzeElementsOfValue(content: string) {
    ...
    const functional = {
      savesTime: this.scoreElement(content, [
        'time-saving',
        'efficient',
        'streamline',
        'automate',
      ]),
      reducesCost: this.scoreElement(content, [
        'cost reduction',
        'save money',
        'affordable',
        'ROI',
      ]),
      reducesEffort: this.scoreElement(content, [
        'simplify',
        'easy',
        'effortless',
        'streamlined',
      ]),
      reducesRisk: this.scoreElement(content, [
        'risk reduction',
        'secure',
        'safe',
        'reliable',
      ]),
      organizes: this.scoreElement(content, [
        'organize',
        'structure',
        'system',
        'framework',
      ]),
      integrates: this.scoreElement(content, [
        'integrate',
        'connect',
        'unify',
        'combine',
      ]),
      connects: this.scoreElement(content, [
        'connect',
        'network',
        'community',
        'relationship',
      ]),
      quality: this.scoreElement(content, [
        'quality',
        'excellent',
        'premium',
        'superior',
      ]),
      variety: this.scoreElement(content, [
        'variety',
        'diverse',
        'multiple',
        'range',
      ]),
      simplicity: this.scoreElement(content, [
        'simple',
        'clear',
        'straightforward',
        'easy',
      ]),
      convenience: this.scoreElement(content, [
        'convenient',
        'accessible',
        'available',
        'ready',
    ...
    const emotional = {
      reducesAnxiety: this.scoreElement(content, [
        'confidence',
        'peace',
        'security',
        'trust',
      ]),
      rewards: this.scoreElement(content, [
        'reward',
        'benefit',
        'advantage',
        'gain',
      ]),
      design: this.scoreElement(content, [
        'design',
        'aesthetic',
        'beautiful',
        'attractive',
      ]),
      fun: this.scoreElement(content, [
        'fun',
        'enjoyable',
        'engaging',
        'exciting',
      ]),
      wellness: this.scoreElement(content, [
        'wellness',
        'health',
        'wellbeing',
        'thrive',
      ]),
      belonging: this.scoreElement(content, [
        'community',
        'belonging',
        'family',
        'team',
    ...
    const lifeChanging = {
      selfActualization: this.scoreElement(content, [
        'potential',
        'growth',
        'transformation',
        'development',
      ]),
      motivation: this.scoreElement(content, [
        'motivation',
        'inspire',
        'empower',
        'drive',
      ]),
      makesMoney: this.scoreElement(content, [
        'revenue',
        'profit',
        'income',
        'financial',
      ]),
      providesAccess: this.scoreElement(content, [
        'access',
        'opportunity',
        'available',
        'reach',
    ...
    const functionalScore = Math.round(
      Object.values(functional).reduce((a, b) => a + b, 0) /
        Object.keys(functional).length
    );
    const emotionalScore = Math.round(
      Object.values(emotional).reduce((a, b) => a + b, 0) /
        Object.keys(emotional).length
    );
    const lifeChangingScore = Math.round(
      Object.values(lifeChanging).reduce((a, b) => a + b, 0) /
        Object.keys(lifeChanging).length
    );

    const overallScore = Math.round(
      (functionalScore + emotionalScore + lifeChangingScore) / 3
    );

    return {
      functional: {
        savesTime: {
          score: functional.savesTime,
          evidence: 'Time-saving language found in content',
        },
        reducesCost: {
          score: functional.reducesCost,
          evidence: 'Cost reduction mentions found',
        },
        reducesEffort: {
          score: functional.reducesEffort,
          evidence: 'Effort reduction language found',
        },
        reducesRisk: {
          score: functional.reducesRisk,
          evidence: 'Risk mitigation language found',
        },
        organizes: {
          score: functional.organizes,
          evidence: 'Organization language found',
        },
        integrates: {
          score: functional.integrates,
          evidence: 'Integration language found',
        },
        connects: {
          score: functional.connects,
          evidence: 'Connection language found',
        },
        quality: {
          score: functional.quality,
          evidence: 'Quality language found',
        },
        variety: {
          score: functional.variety,
          evidence: 'Variety language found',
        },
        simplicity: {
          score: functional.simplicity,
          evidence: 'Simplicity language found',
        },
        convenience: {
          score: functional.convenience,
          evidence: 'Convenience language found',
        },
      },
      emotional: {
        reducesAnxiety: {
          score: emotional.reducesAnxiety,
          evidence: 'Anxiety reduction language found',
        },
        rewards: {
          score: emotional.rewards,
          evidence: 'Reward language found',
        },
        design: { score: emotional.design, evidence: 'Design language found' },
        fun: { score: emotional.fun, evidence: 'Fun language found' },
        wellness: {
          score: emotional.wellness,
          evidence: 'Wellness language found',
        },
        belonging: {
          score: emotional.belonging,
          evidence: 'Belonging language found',
        },
      },
      lifeChanging: {
        selfActualization: {
          score: lifeChanging.selfActualization,
          evidence: 'Self-actualization language found',
        },
        motivation: {
          score: lifeChanging.motivation,
          evidence: 'Motivation language found',
        },
        makesMoney: {
          score: lifeChanging.makesMoney,
          evidence: 'Financial benefit language found',
        },
        providesAccess: {
          score: lifeChanging.providesAccess,
          evidence: 'Access language found',
        },
      },
      socialImpact: {
        selfActualization: {
          score: lifeChanging.selfActualization,
          evidence: 'Social impact language found',
        },
        motivation: {
          score: lifeChanging.motivation,
          evidence: 'Social motivation language found',
        },
        makesMoney: {
          score: lifeChanging.makesMoney,
          evidence: 'Social financial language found',
        },
        providesAccess: {
          score: lifeChanging.providesAccess,
          evidence: 'Social access language found',
        },
      },
      overallScore,
  ...
  private analyzeCliftonStrengths(content: string) {
    ...
    const strategic = this.scoreElement(content, [
      'strategic',
      'planning',
      'vision',
      'future',
    ...
    const executing = this.scoreElement(content, [
      'implement',
      'execute',
      'deliver',
      'results',
    ...
    const influencing = this.scoreElement(content, [
      'influence',
      'persuade',
      'lead',
      'inspire',
    ...
    const relationshipBuilding = this.scoreElement(content, [
      'relationship',
      'connect',
      'collaborate',
      'team',
    ...
    const overallScore = Math.round(
      (strategic + executing + influencing + relationshipBuilding) / 4
    );

    return {
      executing: {
        achiever: {
          score: this.scoreElement(content, [
            'achieve',
            'accomplish',
            'complete',
            'finish',
          ]),
          evidence: 'Achievement language found',
        },
        arranger: {
          score: this.scoreElement(content, [
            'arrange',
          ...
          evidence: 'Arrangement language found',
        },
        belief: {
          score: this.scoreElement(content, [
            'believe',
          ...
          evidence: 'Belief language found',
        },
        consistency: {
          score: this.scoreElement(content, [
            'consistent',
          ...
          evidence: 'Consistency language found',
        },
        deliberative: {
          score: this.scoreElement(content, [
            'careful',
          ...
          evidence: 'Deliberation language found',
        },
        discipline: {
          score: this.scoreElement(content, [
            'discipline',
          ...
          evidence: 'Discipline language found',
        },
        focus: {
          score: this.scoreElement(content, [
            'focus',
        ...
        responsibility: {
          ...
          evidence: 'Responsibility language found',
        },
        restorative: {
          score: this.scoreElement(content, [
            'restore',
          ...
          evidence: 'Restorative language found',
        },
      },
      influencing: {
        activator: {
          ...
          evidence: 'Activation language found',
        },
        command: {
          score: this.scoreElement(content, [
            'command',
          ...
          evidence: 'Command language found',
        },
        communication: {
          score: this.scoreElement(content, [
            'communicate',
          ...
          evidence: 'Communication language found',
        },
        competition: {
          score: this.scoreElement(content, [
            'compete',
          ...
          evidence: 'Competition language found',
        },
        maximizer: {
          score: this.scoreElement(content, [
            'maximize',
          ...
          evidence: 'Maximization language found',
        },
        selfAssurance: {
          score: this.scoreElement(content, [
            'confident',
          ...
          evidence: 'Self-assurance language found',
        },
        significance: {
          score: this.scoreElement(content, [
            'significant',
          ...
          evidence: 'Significance language found',
        },
        woo: {
          score: this.scoreElement(content, [
            'woo',
      ...
      relationshipBuilding: {
        adaptability: {
          ...
          evidence: 'Adaptability language found',
        },
        connectedness: {
          score: this.scoreElement(content, [
            'connect',
          ...
          evidence: 'Connectedness language found',
        },
        developer: {
          score: this.scoreElement(content, [
            'develop',
          ...
          evidence: 'Development language found',
        },
        empathy: {
          score: this.scoreElement(content, [
            'empathy',
          ...
          evidence: 'Empathy language found',
        },
        harmony: {
          score: this.scoreElement(content, [
            'harmony',
          ...
          evidence: 'Harmony language found',
        },
        includer: {
          score: this.scoreElement(content, [
            'include',
          ...
          evidence: 'Inclusion language found',
        },
        individualization: {
          score: this.scoreElement(content, [
            'individual',
          ...
          evidence: 'Individualization language found',
        },
        positivity: {
          score: this.scoreElement(content, [
            'positive',
          ...
          evidence: 'Positivity language found',
        },
        relator: {
          score: this.scoreElement(content, [
            'relate',
          ...
          evidence: 'Relator language found',
        },
      },
      strategicThinking: {
        analytical: {
          ...
          evidence: 'Analytical language found',
        },
        context: {
          score: this.scoreElement(content, [
            'context',
          ...
          evidence: 'Context language found',
        },
        futuristic: {
          score: this.scoreElement(content, [
            'future',
          ...
          evidence: 'Futuristic language found',
        },
        ideation: {
          score: this.scoreElement(content, [
            'idea',
          ...
          evidence: 'Ideation language found',
        },
        input: {
          score: this.scoreElement(content, [
            'input',
        ...
        intellection: {
          ...
          evidence: 'Intellection language found',
        },
        learner: {
          score: this.scoreElement(content, [
            'learn',
          ...
          evidence: 'Learning language found',
        },
        strategic: {
          score: this.scoreElement(content, [
            'strategic',
          ...
          evidence: 'Strategic language found',
        },
      },
      overallScore,
      topThemes: this.getTopThemes({
        themes: { strategic, executing, influencing, relationshipBuilding },
      }),
  ...
  private generateRecommendations(content: string, pageType: string) {
    ...
    if (!content.includes('mission') && !content.includes('purpose')) {
      highPriority.push({
        ...
        description:
          'Define a clear, compelling mission statement that explains why the company exists',
        actionItems: [
          'Write a clear mission statement',
          'Add purpose-driven language to hero section',
        ],
    ...
    if (!content.includes('methodology') && !content.includes('process')) {
      highPriority.push({
        ...
        description:
          'Create a clear methodology or process that differentiates your approach',
        actionItems: [
          'Document your unique process',
          'Add methodology section to website',
        ],
    ...
    if (!content.includes('testimonial') && !content.includes('client')) {
      mediumPriority.push({
        ...
        description:
          'Include specific client testimonials and success stories to build trust',
        actionItems: [
    ...
    if (!content.includes('contact') && !content.includes('phone')) {
      mediumPriority.push({
        ...
        description:
          'Make contact information easily accessible for potential clients',
    ...
    if (
      ...
      mediumPriority.push({
        ...
        description:
          'Include clear call-to-action buttons to guide visitors to next steps',
    ...
    if (pageType === 'home') {
      lowPriority.push({
        ...
        description:
          'Ensure hero section clearly communicates value proposition',
    ...
    return {
      ...
      lowPriority,
      summary:
        'Focus on strengthening the WHY statement and defining your methodology for maximum impact.',
      nextSteps: [
  ...
  private generateSummary(
    ...
    const hasTestimonials =
      content.includes('testimonial') || content.includes('client');
    const hasContact = content.includes('contact') || content.includes('phone');
    const hasCTA =
      content.includes('call to action') || content.includes('get started');
  ...
  private extractTrustSignals(content: string): string[] {
    ...
    if (content.includes('certification'))
      signals.push('Professional certifications');
  ...
  private generateGoldenCircleInsights(
    why: string,
    how: string,
    what: string,
    who: string
    ...
    if (why.length > 50)
      insights.push('Strong WHY statement with clear purpose');
    ...
    if (who.length > 50)
      insights.push('Good client testimonials and target audience definition');

    return insights;
  }

  private generateElementsOfValueInsights(
    functional: any,
    emotional: any,
    lifeChanging: any
  ): string[] {
    ...
    if (functional.savesTime > 7)
      insights.push('Strong time-saving value proposition');
    if (emotional.reducesAnxiety > 7)
      insights.push('Good anxiety reduction messaging');
    if (lifeChanging.selfActualization > 7)
      insights.push('Strong transformation messaging');

    return insights;
  }

  private generateCliftonStrengthsInsights(
    strategic: number,
    executing: number,
    influencing: number,
    ...
    if (executing > 7)
      insights.push('Resonates with execution-focused individuals');
    if (influencing > 7)
      insights.push('Strong influence and leadership appeal');
    if (relationshipBuilding > 7)
      insights.push('Good relationship-building messaging');

    return insights;
  }

  private calculateGoldenCircleScore(
    why: string,
    how: string,
    what: string,
    who: string
