...
export class ThreePhaseAnalyzer {
  ...
  constructor(
    url: string,
    onProgressUpdate?: (phase: string, step: string, progress: number) => void
  ) {
    this.url = url;
  ...
  public async executePhase1(): Promise<Phase1Report> {
    ...
    this.onProgressUpdate?.(
      'Phase 1',
      'Extracting website content and keywords',
      30
    );
    ...
    console.log(
      `✅ Phase 1 completed - Collected ${scrapedContent.wordCount} words`
    );
    return phase1Report;
  }
  ...
  public async executePhase2(
    ...
    const phase2Report: Phase2Report = {
      ...
      summary: {
        ...
        overallFrameworkScore: this.calculateOverallFrameworkScore(
          goldenCircle,
          elementsOfValue,
          b2bElements,
          cliftonStrengths
        ),
        keyStrengths: this.extractKeyStrengths(
          goldenCircle,
          elementsOfValue,
          b2bElements,
          cliftonStrengths
        ),
        keyWeaknesses: this.extractKeyWeaknesses(
          goldenCircle,
          elementsOfValue,
          b2bElements,
          cliftonStrengths
        ),
        valueCentricLanguage: this.extractValueCentricLanguage(
          phase1Report.scrapedContent
        ),
        functionalLanguage: this.extractFunctionalLanguage(
          phase1Report.scrapedContent
        ),
      },
    };
   ...
   */
  public async executePhase3(
    phase1Report: Phase1Report,
    phase2Report: Phase2Report
  ): Promise<Phase3Report> {
    console.log(
    ...
    this.onProgressUpdate?.(
      'Phase 3',
      'Collecting Lighthouse performance data (optional)',
      15
    );
    ...
    try {
      ...
      const lighthouseResponse = await fetch(
        `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${encodeURIComponent(this.url)}&category=PERFORMANCE&category=ACCESSIBILITY&category=BEST_PRACTICES&category=SEO`
      );
      if (lighthouseResponse.ok) {
        ...
        console.log(
          `✅ Lighthouse data collected - Performance: ${lighthouseData.scores.performance}/100`
        );
      }
    } catch (error) {
      console.log(
        '⚠️ Lighthouse not available - will generate recommendations without it'
      );
    }
    ...
    this.onProgressUpdate?.(
      'Phase 3',
      'Collecting Google Trends SEO data (optional)',
      30
    );
    let trendsData = null;
    try {
      // Extract keywords from content for trends analysis
      const contentWords = phase1Report.scrapedContent.content
        .toLowerCase()
        .split(/\s+/);
      const commonWords = [
        'the',
      ...
      if (keywords.length > 0) {
        ...
        const risingQueries =
          relatedParsed.default?.rankedList?.[1]?.rankedKeyword || [];

        trendsData = {
          keyword,
          relatedQueries: topQueries
            .slice(0, 5)
            .map((q: any) => q.topic?.title || q.query),
        ...
        console.log(
          `✅ Google Trends data collected - ${trendsData.relatedQueries.length} related queries found`
        );
      }
    } catch (error) {
      console.log(
        '⚠️ Google Trends not available - will generate recommendations without it'
      );
    }
    ...
    const phase3Report: Phase3Report = {
      ...
      summary: {
        ...
        primaryRecommendations:
          comprehensiveAnalysis?.primaryRecommendations || [],
        quickWins: comprehensiveAnalysis?.quickWins || [],
        longTermImprovements: comprehensiveAnalysis?.longTermImprovements || [],
        performanceOptimizations:
          comprehensiveAnalysis?.performanceOptimizations || [],
        seoImprovements: comprehensiveAnalysis?.seoImprovements || [],
        leadGenerationImprovements:
          comprehensiveAnalysis?.leadGenerationImprovements || [],
    ...
    const dataStatus = lighthouseData
      ? '✅ with Lighthouse'
      : '⚠️ framework-only';
  ...
  private async scrapeWebsiteContent(): Promise<ProductionExtractionResult> {
    ...
    try {
      ...
      console.log(
        `✅ Successfully scraped ${scrapedData.wordCount} words from ${this.url}`
      );
      return scrapedData;
    } catch (error) {
      console.error('❌ Content scraping failed:', error);
      throw new Error(
        `Failed to gather website content: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async runLighthouseAnalysis(): Promise<any> {
    try {
      ...
      if (jsonMatch) {
        ...
        console.log(
          `✅ Lighthouse completed - Performance: ${result.scores?.performance || 'N/A'}`
        );
        return result;
      }
  ...
  private extractKeywordsFromContent(
    ...
    if (content.metaDescription) {
      const descWords = content.metaDescription
        .toLowerCase()
        .replace(/[^\w\s]/g, ' ')
        .split(/\s+/)
        .filter((word) => word.length > 3);
  ...
  private async analyzeElementsOfValue(
    phase1Report: Phase1Report
  ): Promise<any> {
    const { analyzeWithGemini } = await import('./free-ai-analysis');
  ...
  private async analyzeCliftonStrengths(
    phase1Report: Phase1Report
  ): Promise<any> {
    const { analyzeWithGemini } = await import('./free-ai-analysis');
  ...
  private async generateComprehensiveAnalysis(
    ...
    try {
      const { analyzeWithGemini } = await import('./free-ai-analysis');
      const prompt = this.createComprehensiveAnalysisPrompt(
        phase1Report,
        phase2Report
      );
      ...
      console.warn(
        'Gemini analysis failed, using evaluation framework only:',
        error
      );
      return {
        evaluationFramework: evaluationResult,
  ...
  private async generateFinalReport(
    ...
    return {
      ...
      overallScore:
        ...
        this.calculateOverallScore(phase1Report, phase2Report),
      ...
      detailedScores: {
        firstImpression:
          evaluationResult?.categoryScores?.firstImpression?.score || 0,
        coreMessaging:
          evaluationResult?.categoryScores?.coreMessaging?.score || 0,
        technicalPerformance:
          evaluationResult?.categoryScores?.technicalPerformance?.score || 0,
        accessibility:
          evaluationResult?.categoryScores?.accessibility?.score || 0,
        conversionOptimization:
          evaluationResult?.categoryScores?.conversionOptimization?.score || 0,
        contentQuality:
          evaluationResult?.categoryScores?.contentQuality?.score || 0,
        userExperience:
          evaluationResult?.categoryScores?.userExperience?.score || 0,
        socialPresence:
          evaluationResult?.categoryScores?.socialPresence?.score || 0,
        analyticsTracking:
          evaluationResult?.categoryScores?.analyticsTracking?.score || 0,
        securityCompliance:
          evaluationResult?.categoryScores?.securityCompliance?.score || 0,
      },
    };
  }
...
Return JSON format with scores and evidence for each theme.
  ...
  private createComprehensiveAnalysisPrompt(
    phase1Report: Phase1Report,
    phase2Report: Phase2Report
  ): string {
    return `
...
Return JSON format with specific, actionable recommendations.
  ...
  private calculateOverallFrameworkScore(
    goldenCircle: any,
    elementsOfValue: any,
    b2bElements: any,
    cliftonStrengths: any
  ...
  private extractValueCentricLanguage(
    scrapedContent: ProductionExtractionResult
  ): string[] {
  ...
  private extractFunctionalLanguage(
    scrapedContent: ProductionExtractionResult
  ): string[] {
    const content = scrapedContent.content?.toLowerCase() || '';
  ...
  private calculateOverallScore(
    ...
    const phase1Score =
      (phase1Report.summary.seoScore + phase1Report.summary.performanceScore) /
      2;
...
${
  comprehensiveAnalysis?.combinedRecommendations
    ?.slice(0, 5)
