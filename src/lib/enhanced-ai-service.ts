...
export class EnhancedAIService {
  ...
  static async analyzeContent(
    ...
    if (availableProviders.length === 0) {
      ...
      return await EnhancedAIService.analyzeWithContentAnalyzer(
        content,
        contentType,
        url
      );
    ...
    if (!aiProviderService.isProviderAvailable(provider)) {
      ...
      if (fallbackProvider) {
        console.warn(
          `Provider ${provider} not available, using ${fallbackProvider}`
        );
    ...
    try {
      ...
      const result = await aiProviderService.analyzeWithProvider(
        provider,
        request
      );
