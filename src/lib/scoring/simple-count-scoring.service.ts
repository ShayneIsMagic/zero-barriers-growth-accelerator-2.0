...
export class SimpleCountScoringService {
  ...
  private static detectElements(
    ...
    const categories =
      framework === 'b2c' ? this.B2C_CATEGORIES : this.B2B_CATEGORIES;
    ...
    for (const [categoryName, categoryData] of Object.entries(categories)) {
      for (const elementName of categoryData.elements) {
        ...
        elements.push({
          ...
          present,
          evidence: present
            ? this.extractEvidence(content, elementName)
            : undefined,
        });
      }
    }
   ...
   */
  private static isElementPresent(
    content: string,
    elementName: string
  ): boolean {
  ...
  private static buildScoringResult(
    ...
    const totalElements = Object.values(categories).reduce(
    ...
    const overallPercentage =
      totalElements > 0
        ? Math.round((presentElements / totalElements) * 100)
        : 0;

    const categoryDistributions: CategoryDistribution[] = Object.entries(
      categories
