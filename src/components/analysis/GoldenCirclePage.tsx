...
export function GoldenCirclePage() {
  ...
  const saveSnapshot = async () => {
    if (!url.trim() || !(result?.existingData || collectedData)) {
      setLocalError('Please collect content first to save snapshot');
      return;
    }
    ...
    try {
      const existing = result?.existingData ?? collectedData;
      const data = await saveContentSnapshot({
        url: url.trim(),
        title: existing?.title || 'Untitled',
        content: existing?.cleanText || '',
        metadata: {
          wordCount: existing?.wordCount,
          keywords: existing?.extractedKeywords,
