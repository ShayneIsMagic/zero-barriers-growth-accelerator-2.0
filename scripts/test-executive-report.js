...
async function testReportGeneration() {
  ...
  try {
    ...
    const generateResponse = await makeRequest(
      `${BASE_URL}/api/generate-executive-report`,
      {
        method: 'POST',
        headers: {
    ...
    if (generateResponse.statusCode === 200 && generateResponse.data.success) {
      ...
      console.log(
        `   HTML Report Path: ${generateResponse.data.htmlReportPath}`
      );
      ...
      const retrieveResponse = await makeRequest(
        `${BASE_URL}/api/generate-executive-report?reportId=${reportId}`
      );

      if (
        retrieveResponse.statusCode === 200 &&
        retrieveResponse.data.success
      ) {
        console.log('✅ Report retrieval successful');
        console.log(`   Report ID: ${retrieveResponse.data.reportId}`);
        console.log(
          `   Markdown length: ${retrieveResponse.data.markdownContent.length} characters`
        );
        console.log(
          `   HTML available: ${retrieveResponse.data.htmlContent ? 'Yes' : 'No'}`
        );
        ...
        console.log(
          `   Error: ${retrieveResponse.data.error || 'Unknown error'}`
        );
      }
    } else {
      ...
      console.log(
        `   Error: ${generateResponse.data.error || 'Unknown error'}`
      );
    }
...
async function testReportFormats() {
  ...
  try {
    ...
    if (generateResponse.statusCode === 200 && generateResponse.data.success) {
      ...
      console.log(
        `   Has headers: ${reportData.markdownContent.includes('#') ? 'Yes' : 'No'}`
      );
      console.log(
        `   Has tables: ${reportData.markdownContent.includes('|') ? 'Yes' : 'No'}`
      );
      ...
      if (reportData.htmlContent) {
        ...
        console.log(
          `   Has CSS: ${reportData.htmlContent.includes('<style>') ? 'Yes' : 'No'}`
        );
        console.log(
          `   Has JavaScript: ${reportData.htmlContent.includes('<script>') ? 'Yes' : 'No'}`
        );
        console.log(
          `   Print ready: ${reportData.htmlContent.includes('@media print') ? 'Yes' : 'No'}`
        );
      } else {
