...
async function setupAPIKeys() {
  ...
  console.log(
    'Your keys will be stored in .env.local and are protected by .gitignore\n'
  );
  ...
  if (!keys.OPENAI_API_KEY) {
    keys.OPENAI_API_KEY = await question(
  ...
  if (!keys.GEMINI_API_KEY) {
    keys.GEMINI_API_KEY = await question(
      'Enter your Google Gemini API key (or press Enter to skip): '
    );
  }
  ...
  if (!keys.CLAUDE_API_KEY) {
    keys.CLAUDE_API_KEY = await question(
      'Enter your Anthropic Claude API key (or press Enter to skip): '
    );
  }
...
`;
  ...
  if (configuredProviders.length > 0) {
    ...
    console.log(
      '🚀 The app will now use these AI providers for sophisticated analysis!'
    );
  } else {
    console.log(
      '⚠️  No API keys configured. The app will use content-based analysis as fallback.'
    );
  }
