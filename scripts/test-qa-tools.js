...
async function testQATools() {
  ...
  console.log(
    'Testing Puppeteer, Cheerio, and Playwright for content extraction...\n'
  );
    ...
    for (const method of methods) {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:3000/api/scrape-page?url=${encodeURIComponent(testUrl.url)}&method=${method}`
        );
  ...
  console.log('📊 SUMMARY:');
  console.log(
    '• Puppeteer: Best for JavaScript-heavy sites, full browser automation'
  );
  console.log('• Cheerio: Fastest for simple HTML parsing, lightweight');
  console.log(
    '• Playwright: Cross-browser support, good alternative to Puppeteer'
  );
  console.log('');
