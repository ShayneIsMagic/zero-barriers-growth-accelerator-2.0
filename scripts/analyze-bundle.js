...
try {
  ...
  if (fs.existsSync(analyzerPath)) {
    console.log(
      '🌐 Open bundle-analyzer.html in your browser to see detailed bundle breakdown'
    );
  }
  ...
  if (fs.existsSync(outDir)) {
    ...
    if (fs.existsSync(staticDir)) {
      const { execSync } = require('child_process');
      try {
        const sizeOutput = execSync(`du -sh ${staticDir}`, {
          encoding: 'utf8',
        });
        console.log(
          `📁 Static files size: ${sizeOutput.trim().split('\t')[0]}`
        );
      } catch (error) {
