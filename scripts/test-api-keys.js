...
async function testAPIKeys() {
  ...
  console.log(
    `  Gemini: ${keys.gemini ? '✅ Configured' : '❌ Not configured'}`
  );
  console.log(
    `  Claude: ${keys.claude ? '✅ Configured' : '❌ Not configured'}`
  );
  ...
  if (configuredCount === 0) {
    ...
    console.log(
      '💡 Run "node scripts/setup-api-keys.js" to configure your keys.'
    );
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        `🎯 Golden Circle Score: ${result.analysis.goldenCircle.overallScore}/100`
      );
      console.log(
        `💎 Elements of Value Score: ${result.analysis.elementsOfValue.overallScore}/100`
      );
      console.log(
        `🧠 CliftonStrengths Score: ${result.analysis.cliftonStrengths.overallScore}/100`
      );

      // Check if sophisticated analysis was used
      const hasDetailedStructure =
        result.analysis.goldenCircle.why &&
      ...
      if (hasDetailedStructure) {
        ...
        console.log(
          '📝 WHY Statement:',
          result.analysis.goldenCircle.why.statement.substring(0, 100) + '...'
        );
      } else {
  ...
  console.log('\n🎯 Next Steps:');
  console.log(
    '1. If you see sophisticated analysis, your API keys are working!'
  );
