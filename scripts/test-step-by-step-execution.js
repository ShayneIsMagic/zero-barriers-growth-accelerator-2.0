...
async function testStepByStepExecution() {
  ...
  try {
    ...
    const response = await fetch(
      'http://localhost:3000/api/analyze/step-by-step-execution',
      {
        method: 'POST',
        headers: {
    ...
    console.log(
      `📝 Words Scraped: ${data.data?.phase1Data?.scrapedContent?.wordCount || 'N/A'}`
    );
    console.log(
      `🏗️  Lighthouse Score: ${data.data?.phase1Data?.lighthouseData?.scores?.overall || 'N/A'}/100`
    );
    console.log(
      `🔍 PageAudit SEO: ${data.data?.phase1Data?.pageAuditData?.seoScore || 'N/A'}/100`
    );
    console.log(
      `🎯 Overall Score: ${data.data?.summary?.overallScore || 'N/A'}/100`
    );
    console.log('');
    ...
    console.log('='.repeat(25));
    console.log(
      `✅ Phase 1 (Data Collection): ${data.data?.phase1Data ? 'Completed' : 'Failed'}`
    );
    console.log(
      `✅ Phase 2 (Framework Analysis): ${data.data?.phase2Data ? 'Completed' : 'Failed'}`
    );
    console.log(
      `✅ Phase 3 (Strategic Analysis): ${data.data?.comprehensiveAnalysis ? 'Completed' : 'Failed'}`
    );
    console.log(
      `✅ Golden Circle: ${data.data?.phase2Data?.goldenCircle ? 'Completed' : 'Failed'}`
    );
    console.log(
      `✅ Elements of Value: ${data.data?.phase2Data?.elementsOfValue ? 'Completed' : 'Failed'}`
    );
    console.log(
      `✅ B2B Elements: ${data.data?.phase2Data?.b2bElements ? 'Completed' : 'Failed'}`
    );
    console.log(
      `✅ CliftonStrengths: ${data.data?.phase2Data?.cliftonStrengths ? 'Completed' : 'Failed'}`
    );
    console.log('');
    ...
    console.log(
      '   1. Visit http://localhost:3000/dashboard/step-by-step-execution'
    );
