  ...
  if (fs.existsSync(filePath)) {
    ...
    if (content.includes('vi.') && !content.includes("import { vi }")) {
      ...
      if (importMatch) {
        ...
        content = content.slice(0, insertIndex) + '\nimport { vi } from \'vitest\';' + content.slice(insertIndex);
      } else {
...
const unusedVarPatterns = [
  { file: 'src/app/api/analyze/phase1-complete/route.ts', pattern: /(\s+)(competitors)(\s*=\s*)/g, replacement: '$1_$2$3' },
  { file: 'src/app/api/analyze/step-by-step-execution/route.ts', pattern: /(\s+)(stepId)(\s*=\s*)/g, replacement: '$1_$2$3' },
  { file: 'src/app/api/analyze/website/route.ts', pattern: /(WebsiteAnalysisRequest|WebsiteAnalysisResult)(\s*,)/g, replacement: '_$1$2' },
  { file: 'src/app/api/generate-evaluation-guide/route.ts', pattern: /(\s+)(request)(\s*:\s*NextRequest)/g, replacement: '$1_$2$3' },
  { file: 'src/app/api/generate-executive-report/route.ts', pattern: /(\s+)(reject)(\s*:\s*)/g, replacement: '$1_$2$3' },
  { file: 'src/app/api/reports/stats/route.ts', pattern: /(\s+)(request)(\s*:\s*NextRequest)/g, replacement: '$1_$2$3' },
  { file: 'src/app/api/test-db/route.ts', pattern: /(\s+)(request)(\s*:\s*NextRequest)/g, replacement: '$1_$2$3' },
  { file: 'src/app/api/test-prisma-simple/route.ts', pattern: /(\s+)(request)(\s*:\s*NextRequest)/g, replacement: '$1_$2$3' },
  { file: 'src/app/api/test-simple/route.ts', pattern: /(\s+)(request)(\s*:\s*NextRequest)/g, replacement: '$1_$2$3' },
  { file: 'src/app/api/trpc/[trpc]/route.ts', pattern: /(\s+)(request)(\s*:\s*NextRequest)/g, replacement: '$1_$2$3' },
  { file: 'src/app/dashboard/analyze/page.tsx', pattern: /(Badge|isAnalyzing)(\s*,)/g, replacement: '_$1$2' }
];
...
const markdownGeneratorPath = path.join(process.cwd(), 'src/lib/markdown-report-generator.ts');
...
testFilesWithMarkdown.forEach(file => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    // Add proper import
    if (content.includes('MarkdownReportGenerator.generateComprehensiveReport') && !content.includes('MarkdownReportGenerator')) {
      content = content.replace(
...
try {
  ...
  console.log('  ⚠️  TypeScript check found remaining issues, but major fixes applied');
}
