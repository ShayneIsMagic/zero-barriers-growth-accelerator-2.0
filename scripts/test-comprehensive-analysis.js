...
async function testComprehensiveAnalysis() {
  ...
  console.log(
    'Testing WHY, HOW, WHAT, WHO + Bain Elements of Value + CliftonStrengths...\n'
  );
  ...
  try {
    ...
    const scrapeResponse = await fetch(
      `http://localhost:3000/api/scrape-page?url=${encodeURIComponent(testUrl)}`
    );

    if (!scrapeResponse.ok) {
      const errorData = await scrapeResponse.json();
      console.log(
        `❌ Content extraction failed: ${errorData.details || errorData.error}`
      );
      return;
    }
    ...
    console.log(
      `✅ Content extracted: ${scrapeData.wordCount} words, ${scrapeData.imageCount} images`
    );
    ...
    if (analysis.goldenCircle) {
      ...
      if (analysis.goldenCircle.who?.testimonials) {
        analysis.goldenCircle.who.testimonials.forEach((testimonial, index) => {
          console.log(
            `${index + 1}. ${testimonial.client} (${testimonial.company})`
          );
    ...
    if (analysis.cliftonStrengths) {
      ...
      const relationshipBuilding =
        analysis.cliftonStrengths.relationshipBuilding || {};
      ...
      const strategicThinking =
        analysis.cliftonStrengths.strategicThinking || {};
