...
const { ComprehensiveGoogleAnalysisService } = require('../src/lib/comprehensive-google-analysis');
...
async function testComprehensiveGoogleAnalysis() {
  ...
  const testKeywords = ['digital marketing', 'SEO', 'web development', 'content strategy'];
  ...
  try {
    ...
    const googleAnalysisService = new ComprehensiveGoogleAnalysisService(testUrl, testKeywords);
    ...
    console.log(`   Configured: ${results.searchConsole.configured ? '✅' : '❌'}`);
    ...
    if (results.trends.error) {
      ...
      console.log(`   ✅ Keywords analyzed: ${results.trends.data.keywordTrends.length}`);
      console.log(`   ✅ Trending keywords: ${results.trends.data.trendingKeywords.length}`);
      console.log(`   🔥 Top trending: ${results.trends.data.trendingKeywords.slice(0, 3).join(', ')}`);
    }
    console.log();
    ...
    if (results.pageSpeed.error) {
      ...
      console.log(`   ✅ Performance Score: ${results.pageSpeed.data.performance}/100`);
      console.log(`   ✅ SEO Score: ${results.pageSpeed.data.seo}/100`);
      console.log(`   ✅ Accessibility Score: ${results.pageSpeed.data.accessibility}/100`);
      console.log(`   ✅ Best Practices Score: ${results.pageSpeed.data.bestPractices}/100`);
    }
    console.log();

    // Safe Browsing Results
    console.log('4. 🛡️ SAFE BROWSING:');
    console.log(`   Configured: ${results.safeBrowsing.configured ? '✅' : '❌'}`);
    if (results.safeBrowsing.error) {
      console.log(`   Error: ${results.safeBrowsing.error}`);
    } else if (results.safeBrowsing.data) {
      console.log(`   ✅ Safe: ${results.safeBrowsing.data.safe ? '✅ YES' : '❌ NO'}`);
      if (results.safeBrowsing.data.threats.length > 0) {
        console.log(`   ⚠️ Threats detected: ${results.safeBrowsing.data.threats.length}`);
      }
    }
    console.log();
    ...
    console.log(`   Configured: ${results.seoAnalysis.configured ? '✅' : '❌'}`);
    if (results.seoAnalysis.error) {
      ...
      console.log(`   ✅ Meta Description: ${seo.metaDescriptionLength} characters`);
      ...
      console.log(`   ✅ Images with Alt: ${seo.imagesWithAlt}/${seo.totalImages}`);
    }
    console.log();

    // Analysis Summary
    console.log('6. 📊 ANALYSIS SUMMARY:');
    console.log(`   ✅ Tools Configured: ${results.summary.totalToolsConfigured}/${results.summary.totalToolsAvailable}`);
    console.log(`   ✅ Analysis Quality: ${results.summary.analysisQuality.toUpperCase()}`);
    ...
    console.log('==========================================');
    console.log(`✅ Analysis Quality: ${results.summary.analysisQuality.toUpperCase()}`);
    console.log(`✅ Tools Working: ${results.summary.totalToolsConfigured}/${results.summary.totalToolsAvailable}`);
    console.log(`✅ Recommendations Generated: ${results.summary.recommendations.length}`);
    ...
    console.error('❌ Comprehensive Google analysis test failed:', error.message);
    console.log('\n🔧 Troubleshooting:');
